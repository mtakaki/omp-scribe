...
describe("parseTadLine", () => {
  ...
  it("parses a path containing brackets, parens, @, and $ segments", () => {
    const line = "@frontend/app/obras/[slug]/(marketing)/$page.tsx:1-53{~}deps(frontend/lib/analytics.ts)#fire_view_item_event";
    const step = parseTadLine(line);
    expect(step.filePath).toBe("frontend/app/obras/[slug]/(marketing)/$page.tsx");
    expect(step.lineRange).toEqual({ start: 1, end: 53 });
    expect(step.operation).toBe("~");
    expect(step.dependencies).toEqual(["frontend/lib/analytics.ts"]);
    expect(step.intent).toBe("fire_view_item_event");
  });
